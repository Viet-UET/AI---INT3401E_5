...
class AIEngine:
    ...
    def __AlphaBetaPruning(self, gs: GameState, depth, alpha, beta, maximizingPlayer):
        ...
        print(f"DEBUG_AI: Enter AlphaBeta depth={depth}, maxPlayer={maximizingPlayer}, alpha={alpha}, beta={beta}")

        if gs.is_threefold_repetition():
            print(f"DEBUG_AI: Threefold repetition detected at depth {depth}. Returning score 0.")
            return 0 # Trả về điểm hòa

        if depth == 0:
            self.total_nodes_leaf += 1
            score = self.evaluation(gs)
            # << THÊM LOG TRƯỚC KHI RETURN >>
            print(f"DEBUG_AI: Leaf node depth=0, score={score}")
            return score

        start = time.time()
        # << THÊM LOG TRƯỚC KHI GET MOVES >>
        print(f"DEBUG_AI: Getting valid moves for depth={depth}...")
        moves = gs.getValidMoves()
        # << THÊM LOG SAU KHI GET MOVES >>
        print(f"DEBUG_AI: Got {len(moves)} moves for depth={depth}.")
        self.timeGenerateMoves += time.time() - start

        if maximizingPlayer:
            maxEval = - math.inf
            move_count = 0 # << Đếm nước đi đã xét
            for move in moves:
                move_count += 1
                # << THÊM LOG CHO MỖI NƯỚC ĐI >>
                print(f"DEBUG_AI: Depth={depth}, MaxPlayer, Trying move {move_count}/{len(moves)}: {move.getChessNotation()}")
                gs.makeMove(move)
                self.total_node += 1
                eval_score = self.__AlphaBetaPruning(gs, depth - 1, alpha, beta, False)
                gs.undoMove()
                # << THÊM LOG KẾT QUẢ ĐỆ QUY >>
                print(f"DEBUG_AI: Depth={depth}, MaxPlayer, Move {move.getChessNotation()} -> score={eval_score}")
                ...
                if beta <= alpha:
                    ...
                    # << THÊM LOG KHI CẮT TỈA >>
                    print(f"DEBUG_AI: Depth={depth}, MaxPlayer, Beta cutoff: beta={beta} <= alpha={alpha}")
                    break # Thoát vòng lặp sớm
            # << THÊM LOG TRƯỚC KHI RETURN >>
            print(f"DEBUG_AI: Return AlphaBeta depth={depth}, maxPlayer={maximizingPlayer}, maxEval={maxEval}")
            return maxEval
            ...
            for move in moves:
                move_count += 1
                # << THÊM LOG CHO MỖI NƯỚC ĐI >>
                print(f"DEBUG_AI: Depth={depth}, MinPlayer, Trying move {move_count}/{len(moves)}: {move.getChessNotation()}")
                gs.makeMove(move)
                self.total_node += 1
                eval_score = self.__AlphaBetaPruning(gs, depth - 1, alpha, beta, True)
                gs.undoMove()
                # << THÊM LOG KẾT QUẢ ĐỆ QUY >>
                print(f"DEBUG_AI: Depth={depth}, MinPlayer, Move {move.getChessNotation()} -> score={eval_score}")

                if eval_score < minEval:
                    minEval = eval_score
                    if depth == DEPTH:
                        self.bestMove = move # AI Đen có thể không cập nhật bestMove nếu lỗi trước đó
                beta = min(beta, eval_score)
                if beta <= alpha: # Sửa lỗi nhỏ: phải là beta <= alpha
                    self.total_branch_cutoff += 1
                     # << THÊM LOG KHI CẮT TỈA >>
                    print(f"DEBUG_AI: Depth={depth}, MinPlayer, Alpha cutoff: beta={beta} <= alpha={alpha}")
                    break # Thoát vòng lặp sớm
            # << THÊM LOG TRƯỚC KHI RETURN >>
            print(f"DEBUG_AI: Return AlphaBeta depth={depth}, minPlayer={maximizingPlayer}, minEval={minEval}")
            return minEval
